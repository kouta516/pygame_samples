...
mc.setBlock(5, 70, 5,  param.GRASS_BLOCK)
...
display8.init_col(COLOR_ON=param.GRASS_BLOCK, COLOR_OFF=param.STONE)
display8.init_row(X_ORG=-26, Y_ORG=param.Y_SEA + 64, Z_ORG=5, COL_INTV=6)

display9 = LCD_font_mc(mc)
display9.init_col(COLOR_ON=param.GRASS_BLOCK, COLOR_OFF=param.STONE)
...
while running:
    for count in range(16 ** 4):  # 0から65535まで
        ...
        display8.update_col(col=0, code=int(str(dt_now.year)[0]))
        display8.update_col(col=1, code=int(str(dt_now.year)[1]))
        display8.update_col(col=2, code=int(str(dt_now.year)[2]))
        display8.update_col(col=3, code=int(str(dt_now.year)[3]))
        display8.update_col(col=4, code=11)
        display8.update_col(col=5, code=dt_now.month // 10)
        display8.update_col(col=6, code=dt_now.month % 10)
        display8.update_col(col=7, code=11)
        display8.update_col(col=8, code=dt_now.day // 10)
        display8.update_col(col=9, code=dt_now.day % 10)

        display9.update_col(col=0, code=dt_now.hour // 10)
        display9.update_col(col=1, code=dt_now.hour % 10)
        display9.update_col(col=2, code=10)
        display9.update_col(col=3, code=dt_now.minute // 10)
        display9.update_col(col=4, code=dt_now.minute % 10)
        display9.update_col(col=5, code=10)
        display9.update_col(col=6, code=dt_now.second // 10)
        display9.update_col(col=7, code=dt_now.second % 10)

        clock.tick(20)  # FPS, Frame Per Secon8
    # screen.fill(DARK_GRAY)
# infinit loop bottom ----
